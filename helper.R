...
initializeNewModel <- function(ModelName, Author) {
  ...
  NewDir <- file.path(GLOBAL_MODEL_DIR, ModelName)
  dir.create(NewDir)
  ...
  CopyDir <- paste0(GLOBAL_MODEL_DIR, "/templates")
...
initializeCopyModel <- function(ModelName, CopyModelName, Author, CopyScenarios = FALSE) {
  NewDir <- file.path(GLOBAL_MODEL_DIR, ModelName)
  dir.create(NewDir)
  CopyDir <- file.path(GLOBAL_MODEL_DIR, CopyModelName)
  if(CopyScenarios) {
    FilesToCopy_ <- file.path(
...
initializeNewScenario <- function(ModelName, ScenarioName, Concepts_df) {
  ...
  NewDir <- paste0(GLOBAL_MODEL_DIR, "/", ModelName, "/scenarios/", ScenarioName)
  dir.create(NewDir)
  #Create and save a status list
...
initializeCopyScenario <- 
  ...
  #Create directory for scenario
  NewDir <- file.path(GLOBAL_MODEL_DIR, ModelName, "scenarios", ScenarioName)
  dir.create(NewDir)
  #Load the scenario values file to be copied and make conform to model
  CopyFromDir <- file.path(GLOBAL_MODEL_DIR, ModelName, "scenarios", CopyScenarioName)
...
loadScenario <- function(ModelName, ConceptVars_, ScenarioFileName){
  ...
  Dir <- paste0(GLOBAL_MODEL_DIR, "/", ModelName, "/scenarios/", ScenarioFileName)
  #Load the scenario values file to be copied and make conform to model
...
saveScenario <- function(ScenarioData) {
  ...
  ScenarioDir <- file.path(GLOBAL_MODEL_DIR, ModelName, "scenarios", ScenarioName)
...
listScenarios <- function(ModelName) {
  ScenariosDir <- file.path(GLOBAL_MODEL_DIR, ModelName, "scenarios")
  Sc <- dir(ScenariosDir)
  if (length(Sc) == 0) {
    return(list(
      Valid = "",
